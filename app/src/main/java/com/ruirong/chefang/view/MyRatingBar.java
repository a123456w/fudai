        ...
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                if(halfStarMesureWidth ==0){
                    ...
                    Log.d("MyRatingBar","cmeasuredSpace="+measuredSpace);
                    for (int i=0;i<contains.size();i++){
                        starMesureCenterX[i]=(2*i+1)* halfStarMesureWidth +i*measuredSpace;  //计算每颗星星中心到父控件的左边的距离
                        Log.d("MyRatingBar","centerX[i]="+starMesureCenterX[i]);
                    }
                }
            }
        });
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_UP:
                Log.d("MyRatingBar","maxDx="+maxDx+"::maxDy="+maxDy);
                if(maxDx>range||maxDy>range){  //为滑动事件

                }else{  //为点击事件
                    Log.d("MyRatingBar","handleClick");
                    handleClick(downX);
                }
                maxDx=0;
                maxDy=0;
